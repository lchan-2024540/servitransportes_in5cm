...
export class RegisterComponent {
    ...
    get conductorId() { return this.formulario.controls.conductor_id; }
    get terms() { return this.formulario.controls.terms; }

    enviar(): void {
        this.mensajeError = '';

        if (this.rol.value === 'conductor' && !this.conductorId.value) {
            this.conductorId.setErrors({ required: true });
            this.conductorId.markAsTouched();
        }
        ...
        this.cargando = true;
        const { password2, terms, ...valores } = this.formulario.getRawValue();
        const datos = {
            ...valores,
            conductor_id: valores.rol === 'conductor' && valores.conductor_id ? Number(valores.conductor_id) : undefined,
        };
