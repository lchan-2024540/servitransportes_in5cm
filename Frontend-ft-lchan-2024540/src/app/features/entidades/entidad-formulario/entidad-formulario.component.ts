...
export class EntidadFormularioComponent implements OnInit {
    ...
    guardar(): void {
        ...
        const datos = this.convertirTipos(this.limpiarValoresVacios(this.formulario.getRawValue()));
        this.guardando = true;
    ...
    // los <input type="number"> siempre entregan texto; el backend espera numeros reales
    private convertirTipos(valores: Record<string, any>): Record<string, any> {
        const resultado: Record<string, any> = { ...valores };
        for (const campo of this.config.campos) {
            if (campo.tipo === 'number' && resultado[campo.key] !== undefined) {
                const numero = Number(resultado[campo.key]);
                if (!Number.isNaN(numero)) {
                    resultado[campo.key] = numero;
                }
            }
        }
        return resultado;
